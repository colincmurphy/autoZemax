# -*- coding: utf-8 -*-
"""
Created on Mon Jun 27 20:58:23 2022
...
# -*- coding: utf-8 -*-
"""
Created on Mon Jun 27 19:39:57 2022
...
if __name__ == '__main__':
    ...
    #import mirror only file
    path = os.path.join(os.sep, sampleDir,  r'CCAT-prime_ExPoly_prop_prelim-scp.ZOS')
    setup(path)
    
    #Import and set fields
    fieldX, fieldY = autoCD.cdInfo.getFields()
    configureSystem.setFields(fieldX, fieldY)
    
    #Apply ideal coating
    mirrorSurfaces = autoCD.cdInfo.getMirrorSurfaces()
    ...
    amplitudes = []
    averages = []
    angles = range(0, 182, 30)
    for i in range(len(fieldX)):
        for j in range(0, 182, 30):
            angle_averages.append(polarizationRotation.getAverageOfPupilMap(24, 1, (i+1), 27, j ))
        averages.append(np.average(angle_averages))
        for k in range(len(angle_averages)):
            angle_averages[k] = angle_averages[k]-averages[i]
        popt, pcov = grapher.fitToSine(angles, angle_averages)
        print(angles)
        ...
        pcov = []
        reconfigure()
    popt2, pcov2 = grapher.fitToQuad(fieldX, fieldY, amplitudes)
    plt.scatter(fieldX, fieldY, c = amplitudes)
    plt.xlabel("X Displacement (Degrees)")
    plt.ylabel("Y Displacement (Degrees)")
    plt.colorbar()
    plt.title("Period-Averaged FOV Map Phases")
    #grapher.getPlot(popt2, "Period-Averaged FOV Map Phases")
